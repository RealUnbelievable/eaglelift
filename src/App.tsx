                                ...
                                <tbody>
                                    {plannerExercises.map((ex) => (
                                        <tr key={ex.id}>
                                            <td>{ex.name}</td>
                                            <td>
                                                <input type="number" value={ex.reps} onChange={(e) => updateExercise(ex.id, "reps", Number((e.target as HTMLInputElement).value))} />
                                            </td>
                                            <td>
                                                <input type="number" value={ex.sets} onChange={(e) => updateExercise(ex.id, "sets", Number((e.target as HTMLInputElement).value))} />
                                            </td>
                                            <td>
                                                <div>
                                                    {Math.floor(ex.timer / 60).toString().padStart(2, "0")}:{(ex.timer % 60).toString().padStart(2, "0")}
                                                </div>
                                                <input type="number" value={ex.timer} onChange={(e) => setPlannerExercises((prev) => prev.map((item) => item.id === ex.id ? { ...item, timer: +((e.target as HTMLInputElement).value) } : item))} />
